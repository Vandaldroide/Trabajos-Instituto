...
for i in range(len(list)):
    ...
    for j in range(len(list)-i-1):
        suma_j = items[list[j]]["strength"]+items[list[j]]["defense"]+items[list[j]]["agility"]+items[list[j]]["stamina"]
        suma_j1 = items[list[j+1]]["strength"]+items[list[j+1]]["defense"]+items[list[j+1]]["agility"]+items[list[j+1]]["stamina"]
        if suma_j > suma_j1:
            flgc = True
            cn = list[j]
            list[j] = list[j+1]
