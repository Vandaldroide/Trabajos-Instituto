...
for i in range(alto):
    ...
    for j in range(0,ancho,2):
        if ancho%2 != 0 and j >= ancho-2:
            ...
            comprimida_r[i].append((matriz_r[i][j]+matriz_r[i][j+1])//2)
            comprimida_g[i].append((matriz_g[i][j]+matriz_g[i][j+1])//2)
            comprimida_b[i].append((matriz_b[i][j]+matriz_b[i][j+1])//2)
